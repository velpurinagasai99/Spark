...
object addColumnClassify extends App {
//  def classify(avg:Float)={
//    if(avg>4.0) (avg,1) else (avg,0)
//  }
  ...
  //val addedColumnRDD = topMovies.map(x=>(x._1,classify(x._2)))
  ...
  val joinedMap = topMovies.flatMap { case(x, y) =>
    broadcastMovieMap.value.get(x).map { otherValue =>
      (x, (y, otherValue))
    }
  }
